...
class BatchQueue:
    ...
    #  NEW METHOD ADDED: Peek at pending messages without removing them
    def peek_pending(self, limit: int = 5) -> List[Dict]:
        """
        Peek at messages currently in the batch queue without removing them.
        
        Args:
            limit: Maximum number of messages to show
        
        Returns:
            List of pending messages (up to limit)
        """
        with self.lock:
            return list(self.pending_messages)[:limit]
